...
Lets start with the computational complexity of merge. Merge takes in two lists. Let's say the lengths of the lists are  𝑁1  and  𝑁2 . Because we assume the inputs are sorted, merge is very efficient. It looks at the first element of each list and adds the smaller one to the returned list. Every time through the loop we are appending one element to the list, so it will take  𝑁1+𝑁2  iterations until we have the whole list.
